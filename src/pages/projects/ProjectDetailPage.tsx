...
export default function ProjectDetailPage() {
  ...
  const TABS: { key: Tab; label: string; icon: typeof FileText }[] = [
    { key: 'fases',          label: `Fases (${fases.length})`,                     icon: ListChecks },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(15,23,42,0.06)] p-6">
        ...
        <div className="mt-5 pt-5 border-t border-slate-100 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6">
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-medium text-slate-500">Avance del proyecto</span>
                <span className="text-lg font-bold tabular-nums text-slate-900">{progresoActual}%</span>
              </div>
              <ProgressBar value={progresoActual} size="lg" />
              {ultimaActualizacion && (
                <p className="text-[11px] text-slate-400 mt-1.5">
                  Última actualización: {new Date(ultimaActualizacion.fecha).toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs">
              ...
              <span className="flex items-center gap-1.5 text-slate-500">
                ...
                {incidenciasResueltas} resuelta{incidenciasResueltas !== 1 ? 's' : ''}
              </span>
              {plazo && (
                <span className={`flex items-center gap-1.5 ${plazo.urgent ? 'text-red-500 font-medium' : 'text-slate-500'}`}>
          ...
          {actualizaciones.length >= 2 && (
            <div className="flex flex-col items-center justify-center">
              <span className="text-[11px] text-slate-400 mb-1 self-start">Tendencia</span>
